...
pub fn pre_push<G: Git, P: Gpg>(_opt: &PrePush, config: &Config, _local_ref: &str, local_sha: &str, _remote_ref: &str, remote_sha: &str) -> Result<(), Box<dyn Error>> {
    if let Some(c) = &config.verify_git_commits {
        verify_git_commits::<G, P>(c, remote_sha, local_sha)?;
    }
    Ok(())
}

pub fn pre_receive<G: Git, P: Gpg>(config: &Config, old_value: &str, new_value: &str, _ref_name: &str) -> Result<(), Box<dyn Error>> {
    if let Some(c) = &config.verify_git_commits {
        verify_git_commits::<G, P>(c, old_value, new_value)?;
    }
    Ok(())
}
